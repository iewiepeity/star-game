...
| `npc-interaction-events.js` | 10 名主要 NPC 的七類專屬相處／和解事件、選項文案與關係效果 |
...
兩個值得知道的細節：
- **版本號是為了未來的欄位變動準備的**：`persistence.js` 的 `SAVE_VERSION` 是目前寫出的版本，`SUPPORTED_SAVE_VERSIONS` 列出仍可讀取的舊版；結構變更時應在 `core/migrations.js` 寫遷移，不可直接丟棄既有玩家進度。目前版本為 v12，除了既有的隱藏好感與唯一伴侶欄位，也會替舊關係資料補上隱藏敵意、敵意歷史及 NPC 相處記憶。
...
- 逐日事件畫面只在 NPC 專屬相處時載入該人物的半身圖；一般訓練、休息與工作仍以場景為主，避免沒有出場人物時硬塞立繪。
