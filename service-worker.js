const CACHE="star-game-runtime-v1.47.0";
...
self.addEventListener("install",event=>event.waitUntil(trackCacheTask(caches.open(CACHE).then(cache=>cache.addAll(SHELL)))));
...
self.addEventListener("message",event=>{
 ...
 if(event.data?.type==="STOP_GAME_CACHING"){
  cachingStopped=true;
  event.waitUntil((async()=>{while(cacheTasks.size)await Promise.allSettled([...cacheTasks]);event.ports[0]?.postMessage({type:"GAME_CACHING_STOPPED"});})());
 }
});
async function networkFirst(request){const cache=await caches.open(CACHE);try{const response=await fetch(request);if(response?.ok)await cache.put(request,response.clone());return response}catch{const cached=await cache.match(request,{ignoreSearch:true});if(cached)return cached;if(request.mode==="navigate")return cache.match(new URL(request.url).pathname.endsWith("pixel.html")?"./pixel.html":"./index.html");throw new Error("offline")}}
async function staleWhileRevalidate(request,event){const cache=await caches.open(CACHE),cached=await cache.match(request),refresh=trackCacheTask(fetch(request).then(async response=>{if(response?.ok)await cache.put(request,response.clone());return response}).catch(()=>null));if(cached){event.waitUntil(refresh);return cached}const response=await refresh;if(response)return response;throw new Error("offline")}
...
self.addEventListener("fetch",event=>{const url=new URL(event.request.url);if(cachingStopped||!isCacheable(event.request,url))return;const dynamic=event.request.mode==="navigate"||["script","style"].includes(event.request.destination)||url.pathname.endsWith(".json");const task=trackCacheTask(dynamic?networkFirst(event.request):staleWhileRevalidate(event.request,event));event.waitUntil(task);event.respondWith(task)});
...
async function cachePixelWorld(source) {
 ...
 await Promise.all(Array.from({length:4},async()=>{
  while(!cachingStopped&&index<entries.length){const entry=entries[index++];try{if(!await cache.match(entry.url)){const response=await fetch(entry.url);if(!response.ok)throw new Error("asset");await cache.put(entry.url,response);}}catch{failures++;}done++;if(done%10===0)report();}
 }));
 if(cachingStopped)return;
 if(!failures)await cache.put("./pixel-offline-ready",new self.Response(manifest.version));
 report(true);
}
self.addEventListener("message",event=>{
 if(cachingStopped||event.data?.type!=="PIXEL_CACHE_ALL")return;
 if(!pixelDownload)pixelDownload=trackCacheTask(cachePixelWorld(event.source).catch(()=>event.source?.postMessage({type:"PIXEL_OFFLINE_PROGRESS",percent:0,done:true,message:"離線內容未完成，請確認連線與儲存空間後重試。"})).finally(()=>{pixelDownload=null;}));
