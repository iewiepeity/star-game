...
test("NPC 專屬事件與戀愛事件會依角色與關係階段出現",()=>{fresh();assert.ok(NPC_STORY_CONTENT.tangtang?.hook&&NPC_STORY_CONTENT.tangtang?.private&&NPC_STORY_CONTENT.tangtang?.romance);meetNpc("tangtang");adjustRelationship("tangtang",{closeness:90,trust:85,affection:45});state.eventQueue=[];queueNpcStoryEvents();assert.ok(state.eventQueue.some(x=>x.event.id==="npc-story-tangtang:stage:bonded"));advanceRomance("tangtang","interested");state.eventQueue=[];queueNpcStoryEvents();assert.ok(state.eventQueue.some(x=>x.event.id.startsWith("npc-romance-tangtang:romance:interested:")))});
