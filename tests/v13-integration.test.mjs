...
test("NPC 專屬事件與戀愛事件會依角色與關係階段出現",()=>{fresh();meetNpc("tangtang");adjustRelationship("tangtang",{closeness:90,trust:85});state.eventQueue=[];queueNpcStoryEvents();assert.ok(state.eventQueue.some(x=>x.event.text.includes("Demo")||x.event.text.includes("舞台")));advanceRomance("tangtang","interested");state.eventQueue=[];queueNpcStoryEvents();assert.ok(state.eventQueue.some(x=>x.event.id.includes("npc-romance")))});
