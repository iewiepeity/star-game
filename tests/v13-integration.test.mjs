...
test("NPC 專屬事件與戀愛事件會依角色與關係階段出現",()=>{fresh();meetNpc("tangtang");adjustRelationship("tangtang",{closeness:90,trust:85});state.eventQueue=[];queueNpcStoryEvents();assert.ok(state.eventQueue.some(x=>x.event.text===NPC_STORY_CONTENT.tangtang.romance));advanceRomance("tangtang","interested");state.eventQueue=[];queueNpcStoryEvents();assert.ok(state.eventQueue.some(x=>x.event.id.includes("npc-romance")))});
