...
async function open(page, app) {
  await page.evaluate(async (app) => {
    ...
    (await import("/src/core/app-navigation.js")).openApp(state, app);
...
test("工作入口說明來源，安排公司探訪不會提早取得打工", async ({ page }) => {
  ...
  await expect(page.locator(".jobs-page > h2")).toHaveText(
    "目前信箱裡沒有工作",
  );
  ...
  await page
    .locator('.work-discovery [data-city-shortcut="film_company"]')
    .click();
  await expect(page.locator(".city-preview")).toContainText("首次到訪可登記");
  await page
    .locator('.city-preview [data-city-confirm="film_company"]')
    .click();
  ...
  const progress = await page.evaluate(async () => {
    ...
    return {
      ...
      shifts: state.partTimeShifts,
    };
  });
  expect(progress.visits[1]).toContain("film_company");
  expect(progress.shifts.film_runner).toBeUndefined();
});
test("已登記打工可從行程表安排，完成才領薪與累積第三次經驗", async ({
  page,
}, info) => {
  await create(page);
  ...
  await page.evaluate(async () => {
    const { state } = await import("/src/core/state.js");
    state.visitedLocationsByWeek = { 1: ["film_company"] };
    state.partTimeShifts = { film_runner: 2 };
    state.week = 4;
    state.appOpen = "jobs";
    (await import("/src/render.js")).render();
  });
  ...
  const before = await page.evaluate(async () => {
    const { state } = await import("/src/core/state.js");
    return { money: state.money, count: state.partTimeShifts.film_runner };
  });
  expect(before.count).toBe(2);
  await page.locator("#begin-week").click();
  await expect(page.locator(".day-result")).toContainText(
    "已完成這裡的三次打工",
  );
  const after = await page.evaluate(async () => {
    const { state } = await import("/src/core/state.js");
    const { JOB_CATALOG } = await import("/src/data/jobs.js");
    const job = JOB_CATALOG.find((j) => j.category === "電影" && j.stars === 2);
    const { canAccessJob } = await import("/src/logic/industry.js");
    return {
      money: state.money,
      count: state.partTimeShifts.film_runner,
      access: canAccessJob(job),
    };
  });
  expect(after.money).toBeGreaterThanOrEqual(before.money + 1500);
  expect(after.count).toBe(3);
  expect(after.access.ok).toBe(true);
  expect(after.access.source.type).toBe("experience");
  await expect
    .poll(() =>
      page.evaluate(
        () =>
          JSON.parse(localStorage.getItem("star-game-save") || "{}").state
            ?.partTimeShifts?.film_runner,
      ),
    )
    .toBe(3);
  ...
  await expect
    .poll(() =>
      page.evaluate(
        async () =>
          (await import("/src/core/state.js")).state.partTimeShifts.film_runner,
      ),
    )
    .toBe(3);
});
test("舊排程未到訪的公司打工不會領薪、增加能力或工作經驗", async ({ page }) => {
  ...
  const before = await page.evaluate(async () => {
    const { state } = await import("/src/core/state.js");
    state.schedule[0] = "film_runner";
    state.appOpen = "planner";
    (await import("/src/render.js")).render();
    return { money: state.money, stats: state.stats };
  });
  await page.locator("#begin-week").click();
  await expect(page.locator(".day-result")).toContainText(
    "沒有獲得工作收入或經驗",
