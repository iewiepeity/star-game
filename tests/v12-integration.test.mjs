...
import{tickScandals}from"../src/logic/scandal-engine.js";
...
test("炎上會降低品牌市場並由醜聞引擎建立公關回應事件",()=>{fresh();state.rep.爭議度=760;state.rep.可信度=260;const p=tickPublicOpinion([{heat:500}]);assert.equal(p.state,"scandal");assert.equal(brandEligible(4),false);assert.ok(jobMarketModifier("廣告")<0);tickScandals();assert.ok(state.eventQueue.some(x=>x.event.id?.startsWith("scandal-choice")));assert.ok(state.industryNews.some(n=>n.category==="輿論"))});
test("爆紅會提高工作市場並建立曝光選擇",()=>{fresh();state.fame=500;state.rep.路人緣=900;state.rep.國民度=700;state.rep.業界評價=700;state.rep.話題度=900;state.rep.可信度=700;state.publicOpinion.heat=85;const p=tickPublicOpinion([{heat:600}]);assert.equal(p.state,"viral");assert.ok(jobMarketModifier("廣告")>0);assert.ok(state.eventQueue.some(x=>x.event.id?.startsWith("viral-response")))});
