...
export function settingsApp() {
  ...
  return `<div class="inside-page settings-page"><div class="inside-title"><div><span>GAME SETTINGS</span><h2>遊戲設定</h2></div><p>顯示與播放偏好會保留在這台裝置，不受讀檔影響</p></div>${state.saveNotice ? `<div class="wardrobe-notice">${esc(state.saveNotice)}</div>` : ""}
    ...
    ${section("AUDIO", "音樂與音效", "音樂會依房間、事件、行程與結局切換；首次點擊後開始播放。所有聲音皆為遊戲即時合成，可離線使用。", `${slider("musicVolume", "背景音樂", prefs.musicVolume)}${slider("sfxVolume", "操作音效", prefs.sfxVolume)}<div class="audio-previews" aria-label="試聽音效"><button data-preview-sfx="message">訊息</button><button data-preview-sfx="success">成功</button><button data-preview-sfx="warning">警告</button><button data-preview-sfx="reward">獎勵</button></div><button class="settings-link" data-audio-muted>${prefs.audioMuted ? "開啟聲音" : "全部靜音"}</button>`)}
