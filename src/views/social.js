...
function feed(){
 ...
 const npc=state.knownPeople.filter(id=>NPCS[id]&&NPC_SOCIAL_COPY[id]).map((id,i)=>{const n=NPCS[id];return{id:`npc-${id}`,name:n.name,badge:n.job,avatar:n.avatar,image:n.head,text:NPC_SOCIAL_COPY[id],likes:86+i*37+state.week*3,comments:["工作辛苦了！","期待下次更新。"]}});
