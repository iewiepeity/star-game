...
function activeJobAlerts() {
  return Object.values(state.activeJobs || {})
    ...
    .map((record) => {
      ...
      return job
        ? `<button class="contract-alert" data-open-app="jobs"><span>進行中通告</span><b>${esc(job.title)}</b><strong>剩餘 ${record.remainingSessions} 次・第 ${record.deadlineWeek} 週截止</strong><small>指定工作日：${job.workDays.map((index) => DAYS[index]).join("、")}</small><em>查看合約 →</em></button>`
        : "";
    })
    .join("");
}
...
function plannerCommandBar(fat,spend){
  const urgent=Object.values(state.activeJobs||{}).filter(r=>r.stage==="active"&&r.deadlineWeek-state.week<=1).length;
  const risk=state.health<=45||fat>=85?"高風險：先保住健康":urgent?`${urgent} 份通告接近違約`:spend>state.money?"本週預算不足":"排程目前可執行";
  return `<section class="planner-command-bar"><header><span>WEEK CONTROL</span><b>${risk}</b></header><div><span>健康 <b>${state.health}</b></span><span>體力 <b>${state.stamina}</b></span><span>心情 <b>${state.mood}</b></span><span>週末疲勞 <b>${fat}</b></span><span>剩餘資金 <b>${money(state.money-spend)}</b></span></div><small>點選日期後再選行程；試鏡、創作、人物與經紀人互動會顯示為「已預約」的重要行程。</small></section>`;
}
  ...
  return `<div class="planner-app">${plannerCommandBar(fat,spend)}${
    forced
      ...
      : `${activeJobAlerts()}<div class="week-brief"><div><span>本週任務</span><b>${task.label}</b><small>${task.desc}</small></div><strong>獎勵<br><b>${task.rewardText}</b></strong></div><section class="focus-panel"><div class="focus-row"><span>本週策略</span>${Object.entries(
          ...
          .join(
            "",
        )}</div><div class="focus-explainer"><i>${focus.icon}</i><span><small>已套用到本週所有相關結算・${focus.badge}</small><b>${focus.label}</b><em>${focus.note}</em></span><strong>${state.focus==="growth"?"例：訓練原本＋5，本週會變＋6":state.focus==="fame"?"例：公開試鏡成功率直接＋5%，成功再多 2 知名度":"例：初次認識與正向互動會額外增加關係"}</strong></div></section>`
...
export function calendarDay(id, i, forced = false) {
  ...
  return `<button class="calendar-cell ${!forced && state.selectedDay === i ? "selected" : ""} ${dueDay ? "due-day" : ""} ${id === "agency_interview" || id === "personal_task" ? "important-day" : ""}" data-day="${i}" ${forced ? "disabled" : ""}><span>${i + 1}<small>週${SHORT[i]}</small></span><i class="${a.type}">${a.icon}</i><b>${forced ? "強制休養" : assignedJob ? assignedJob.title : personal?.label || interviewLabel || freePlace || a.short}</b><em>${forced ? "醫療鎖定" : assignedJob ? "正式通告" : personal ? `${personal.cost ? money(personal.cost) : "已預約"}` : id === "agency_interview" ? "不收費・重要行程" : a.cost ? money(a.cost) : "免費"}</em></button>`;
}
export function activityPicker() {
  ...
  const assistant = `<details class="schedule-assistant"><summary><span>✦ 排程小幫手</span><b>少排一點行政，多活一點人生</b><em>展開</em></summary><div class="preset-list">${Object.entries(
    SCHEDULE_PRESETS,
  )
    .map(
      ([id, preset]) =>
