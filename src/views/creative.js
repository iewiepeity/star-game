import { workEchoTimeline } from "./work-echoes.js";
...
function projectCard(p, collapsedProjectIds) {
  ...
  const releaseStory = p.status === "released" ? p.storyHistory?.findLast(entry => entry.phase === "release" && entry.text)?.text || creativeStory(p, "release") : "";
