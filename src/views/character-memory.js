import { BOND_LABELS } from "../logic/romance-experiences.js";
...
export function characterMemoryPanel(npcId, game = state) {
  ...
  return `<section class="npc-shared-memories character-memory"><header><span>THINGS WE REMEMBER</span><h3>讓對方更懂你</h3></header><article><b>一起走過的事</b><p>${memory.bonds?.length ? memory.bonds.map(id => esc(BOND_LABELS[id])).join("、") : "關心支持、深入談心、工作交流或履行約定，都能慢慢留下共同經歷。"}</p></article><p>把喜好和界線親口告訴對方，也可以隨時重新說明。</p>${MEMORY_TOPICS.map(topic => {
