...
export function resultView(){const r=state.runnerResult;return `<div class="day-result ${r.success?"success":"fail"}"><span>${r.success?"RESULT・SUCCESS":"RESULT・EXPERIENCE"}</span><h2>${r.title}</h2><div class="day-result-copy">${r.text}</div><div class="day-result-stats"><b>💰 ${money(state.money)}</b><b>★ 知名度 ${state.fame}</b><b>♡ 粉絲 ${state.fans}</b></div><button class="main-btn" id="next-day">${state.runnerDay===6?"查看本週總結":"前往"+DAYS[state.runnerDay+1]} →</button></div>`}
