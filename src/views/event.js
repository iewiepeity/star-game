...
export function eventView(){
 ...
 if(!item&&state.eventOutcome){const r=state.eventOutcome,effects=r.effects?.length?r.effects.map(x=>`<li>${x}</li>`).join(""):`<li>這次選擇已寫入本輪經歷，可能成為後續事件的條件。</li>`;return`<main class="narrative-event-screen"><div class="event-ambient" aria-hidden="true"></div><section class="narrative-event-card event-outcome"><div class="event-card-rule"><span>EVENT RESULT</span><em>結果已記錄</em></div><div class="event-outcome-body"><i>✓</i><span>${esc(r.kind||"事件結果")}</span><h1>${esc(r.title||"事件處理完成")}</h1><div class="event-choice-made"><small>你的選擇</small><b>${esc(r.choiceLabel||"確認這段經歷")}</b></div><p>${esc(r.outcome||"這次決定已經留下影響，未來可能在其他事件中再次被提起。")}</p><section><b>本次影響</b><ul>${effects}</ul></section>${r.hasFollowUp?`<aside>✦ 這段故事尚未結束，後續會在符合條件時出現。</aside>`:""}<button id="event-continue" class="main-btn">確認結果・回到房間 →</button></div></section></main>`}
