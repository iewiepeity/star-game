...
import{npcStoryStatus}from"../logic/npc-engine.js";
...
export function npcApp(){
 ...
 const current=ids.includes(state.selectedNpc)?state.selectedNpc:ids[0],npc=NPCS[current],p=npc.profile,rel=state.relationships[current]||{closeness:0,trust:0,romance:"none"},story=npcStoryStatus(current),familiar=state.familiarNpcs.includes(current),artView=state.npcArtView==="full"?"full":"bust",art=artView==="full"?npc.portrait:npc.bust;
 return `<div class="npc-dossier">
  ...
  <section class="npc-profile">
   ...
   <div class="npc-profile-copy">
    ...
    <section class="relationship-panel"><div><span>好感</span><i><b style="width:${rel.closeness}%"></b></i><strong>${rel.closeness}／100</strong></div><div><span>信任</span><i><b style="width:${rel.trust}%"></b></i><strong>${rel.trust}／100</strong></div><small>人物階段：${esc(story.stage.label)}・關係狀態：${rel.romance==="none"?"尚未定義":esc(rel.romance)}。</small>${story.events.length?`<p>已解鎖 ${story.events.length} 段人物事件；後續會由合作、探索與選擇繼續推進。</p>`:""}</section>
   </div>
  </section>
 </div>`;
}
