...
function guidedJourney() {
  ...
  return `<section class="journey-guide"><header><span>CAREER GUIDE・第一份作品</span><b>${journey.label}</b><p>${journey.text}</p></header><div><button ${journey.location && journey.app === "map" ? `data-city-shortcut="${journey.location}"` : `data-open-app="${journey.app}"`}>${journey.action}<i>→</i></button></div></section>`;
}
