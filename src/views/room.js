...
import { APP_META, APP_LIBRARY_IDS, APP_CATEGORIES, APP_CATEGORY_LABELS, normalizeDockIds, appIcon } from "./app-icons.js";
...
export function tabletHome() {
  ...
  const appQuery=(state.appQuery||"").trim().toLocaleLowerCase("zh-Hant"),appCategory=APP_CATEGORY_LABELS.includes(state.appCategory)?state.appCategory:"全部";
  const visibleApps=APP_LIBRARY_IDS.filter(id=>(appCategory==="全部"||APP_CATEGORIES[id]===appCategory)&&(!appQuery||`${APP_META[id].label} ${APP_META[id].title} ${APP_META[id].note}`.toLocaleLowerCase("zh-Hant").includes(appQuery)));
  ...
  return `<div class="tablet-home"><header><span>第 ${yearOf()} 年・第 ${weekInYear()} 週</span><b>${esc(state.name)}，早安！</b><small>${esc(phase.label)}・${esc(phase.goal)}</small></header>${agencyHomeCard()}${weeklyCommandCenter()}<button class="next-action" data-open-app="planner"><span>本週主要行動</span><b>安排第 ${state.week} 週行程</b><small>預計支出 ${money(budget())}</small><i aria-hidden="true">→</i></button><section class="app-library ${state.dockEditing?"editing":""}"><header><div><span>${state.dockEditing?"CUSTOMIZE DOCK":"APP LIBRARY"}</span><b>${state.dockEditing?"挑選六個常用 App":"全部 App"}</b></div>${tools}</header>${state.dockEditing?`<p class="dock-editor-note">點選 App 加入或移出下方快捷列；排列順序就是你加入的順序。${state.dockNotice?`<strong>${esc(state.dockNotice)}</strong>`:""}</p>`:`<div class="app-library-tools"><label><span class="sr-only">搜尋 App</span><input type="search" data-app-query data-focus-key="app-query" value="${esc(state.appQuery||"")}" placeholder="搜尋 App 或功能" aria-label="搜尋 App"></label><nav aria-label="App 分類">${APP_CATEGORY_LABELS.map(category=>`<button data-app-category="${category}" aria-pressed="${appCategory===category}" class="${appCategory===category?"active":""}">${category}</button>`).join("")}</nav></div>`}<div class="home-launchers">${(state.dockEditing?APP_LIBRARY_IDS:visibleApps).map(appTile).join("")}</div>${!state.dockEditing&&!visibleApps.length?`<div class="app-library-empty"><b>沒有找到符合的 App</b><button data-clear-app-filters>清除搜尋與分類</button></div>`:""}</section></div>`;
}
...
export function appWindow() {
  ...
  const returnButton=state.appReturnContext?.app&&state.appReturnContext.app!==state.appOpen?`<button class="window-return" data-return-app="${esc(state.appReturnContext.app)}">← ${esc(state.appReturnContext.label||"返回上一頁")}</button>`:"";
  const closeGuard=state.appCloseConfirm?`<section class="app-close-guard" role="alertdialog" aria-modal="true" aria-labelledby="app-close-title"><b id="app-close-title">要關閉目前畫面嗎？</b><p>尚未完成的輸入與確認流程會保留，但你需要重新開啟 App 才能繼續。</p><div><button data-cancel-app-close>繼續操作</button><button class="danger" data-confirm-app-close>保留草稿並關閉</button></div></section>`:"";
  return `<div class="app-backdrop" data-close-app></div><section class="app-window ${state.appOpen}" role="dialog" aria-modal="true" aria-labelledby="app-window-title" tabindex="-1"><header class="window-bar">${returnButton}<div class="window-icon tone-${meta.tone}">${appIcon(state.appOpen)}</div><div><h2 id="app-window-title">${meta.title}</h2><p>${meta.note}</p></div>${state.appOpen === "planner" ? `<button class="window-start" id="begin-week">開始這週 →</button>` : ""}<button class="window-close" data-close-app aria-label="關閉${meta.title}">×</button></header><div class="window-body">${body}</div>${closeGuard}</section>`;
}
