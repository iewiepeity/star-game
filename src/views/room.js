...
function weeklyCommandCenter() {
  const active = Object.values(state.activeJobs || {})
    ...
    scheduled = Object.values(state.scheduledActivities || {}).filter(
      (task) => task.week === state.week && task.status === "scheduled",
    ),
    messages = (state.npcMessages || []).filter(
    ...
    queued =
      ...
      (state.queuedEvents?.filter((item) => item.dueWeek <= state.week + 1)
        .length || 0),
    market = state.worldMarket?.headline || "市場本週相對平穩";
  return `<section class="home-briefing"><header><span>THIS WEEK・行前簡報</span><b>${job ? `《${esc(job.title)}》第 ${urgent.deadlineWeek} 週截止` : scheduled.length ? `${scheduled.length} 件個人安排已排入行程` : "目前沒有迫近的合約期限"}</b></header><div><button data-open-app="jobs"><small>工作期限</small><b>${job ? `剩餘 ${urgent.remainingSessions} 次／${urgent.deadlineWeek - state.week} 週` : `${active.length} 份執行中`}</b></button><button data-open-app="people"><small>人物訊息</small><b>${messages ? `${messages} 則未讀` : "目前已讀完"}</b></button><button data-open-app="log"><small>市場風向</small><b>${esc(market)}</b></button><button data-open-app="log"><small>劇情待辦</small><b>${queued ? `${queued} 件依序等待` : "本週清爽"}</b></button></div></section>`;
}
...
export function tabletDock() {
  return `<nav class="tablet-dock">${[
    ...
    .map(
      ([id, icon, label]) =>
        `<button data-open-app="${id}"><i>${icon}</i>${label}${id === "achievements" && state.achievementNotifications?.length ? `<b class="app-badge">${state.achievementNotifications.length}</b>` : ""}</button>`,
    )
    .join("")}</nav>`;
}
export function appWindow() {
  const meta = {
    planner: ["▦", "行程與工作", "安排七天行程、策略與預算"],
    stats: ["◫", "能力資料", "檢視公開能力與隱藏特質"],
    creative: ["✎", "創作工作室", "創作、修改並向產業公司投稿"],
    wardrobe: ["♢", "造型衣櫃", "切換造型與能力加成"],
    people: ["♡", "手機與人際", "查看聯絡人和關係進度"],
    log: ["≡", "星途紀錄", "回顧每週成果與重大選擇"],
    world: ["◉", "娛樂圈週報", "整合玩家狀態、市場、作品、人物與後台世界"],
    achievements: ["♛", "星途成就", "收藏旅程、工作、人際與生活里程碑"],
    map: ["⌖", "星望市地圖", "前往城市地點與娛樂產業公司"],
    jobs: ["▤", "工作信箱", "管理公開徵選、推薦與指名邀約"],
    npc: ["◈", "人物檔案", "已認識人物的資料與關係"],
    social: ["◎", "星光社群", "發布近況並查看動態"],
    forum: ["☷", "星談論壇", "娛樂討論與熱門話題"],
    agency: ["◆", "經紀公司", "投遞、面談與簽約進度"],
    save: ["💾", "存檔管理", "管理存檔與安全備份"],
    settings: ["⚙", "遊戲設定", "存讀檔、顯示偏好與遊戲流程"],
  }[state.appOpen];
  const views = {
    planner: plannerApp,
    stats: statsApp,
