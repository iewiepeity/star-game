...
export function tabletDock() {
  return `<nav class="tablet-dock">${[["planner", "▦", "行程"], ["stats", "◫", "能力"], ["achievements", "♛", "成就"], ["people", "♡", "人際"], ["log", "≡", "紀錄"]].map(([id, icon, label]) => `<button data-open-app="${id}"><i>${icon}</i>${label}${id === "achievements" && state.achievementNotifications?.length ? `<b class="app-badge">${state.achievementNotifications.length}</b>` : ""}</button>`).join("")}</nav>`;
}
export function appWindow() {
  ...
  const views = { planner: plannerApp, stats: statsApp, creative: creativeApp, wardrobe: wardrobeApp, people: peopleApp, log: logApp, world: worldApp, achievements: achievementsApp, map: mapApp, jobs: jobsApp, npc: npcApp, social: socialApp, forum: forumApp, agency: agencyApp, save: saveApp, settings: settingsApp };
