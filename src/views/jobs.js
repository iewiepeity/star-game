import { normalizeWorkEchoes, releasedEchoWorks } from "../logic/work-echoes.js";
...
function requirementRows(job) {
  return qualification(job)
    .rows.map((row) => {
      ...
      return `<span class="${row.met ? "met" : "unmet"}"><b>${row.name}・${row.core ? "必要能力" : "加分能力"}</b><small>目前 ${row.current}${bonus ? `（服裝＋${bonus}）` : ""}／需求 ${row.required}</small><em>${row.met ? "符合" : row.core ? "未達" : "可帶差距試鏡"}</em></span>`;
    })
    .join("");
}
const sourceLabel = (job) => {
  ...
  if (s.type === "legacy") return `《${esc(releasedEchoWorks().find(w => w.id === s.sourceWorkId)?.title || "舊作")}》帶來的窗口`;
  if (s.type === "network")
...
function workEchoOffers() {
  const offers = normalizeWorkEchoes(state.workEchoes).opportunities.filter(o => !o.usedWeek && o.expiresWeek >= state.week && (!state.activeJobs[o.jobId] || state.activeJobs[o.jobId].stage === "available"));
  return offers.length ? `<section class="manager-card"><h3>舊作帶來的新窗口</h3><p>已有作品讓製作方願意再談；仍需符合資格並由你自行安排試鏡。</p>${offers.map(o => `<p>《${esc(releasedEchoWorks().find(w => w.id === o.workId)?.title || "舊作")}》→ <button data-select-job="${o.jobId}">${esc(JOB_BY_ID[o.jobId]?.title || o.jobId)}</button>・第 ${o.expiresWeek} 週前・作品討論準備＋4</p>`).join("")}</section>` : "";
}
export function jobsApp(options = {}) {
  ...
  if (!allJobs.length && !sequels)
    return `<div class="jobs-page"><h2>目前信箱裡沒有工作</h2><p>新人可先接市民服務台的活動零工，或去公司登記打工、詢問公開徵選。完成同公司三次打工或累積相關作品後，窗口才會持續提供消息；人脈引薦與經紀人推薦也會逐步帶來機會。</p>${workDiscovery(options)}</div>`;
  if (!jobs.length)
