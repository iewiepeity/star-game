...
export function createStorageUI(api) {
  ...
  function extras() {
    return `<section class="transfer-tools"><h3>把旅程帶著走</h3><p class="tiny-note">匯出 JSON，在另一個瀏覽器或裝置匯入。也能匯入舊版存檔；會先預覽內容。</p><div class="buttons">${storage.hasRecoveryCopy() ? '<button data-storage-raw>下載復原前原始資料</button>' : ""}<button data-storage="export">匯出目前旅程</button><button data-storage="import">匯入存檔</button><button data-storage="legacy">搬入這台裝置的舊版存檔</button><button data-storage="backup" ${storage.readBackup().state ? "" : "disabled"}>還原搬移前備份</button></div><input type="file" accept=".json,application/json" id="pixel-import-file" hidden></section><details class="save-maintenance"><summary>管理手動存檔 · 刪除與還原</summary>${[1, 2, 3, 4, 5].map((slot) => `<div class="save-row"><b>位置 ${slot}</b><button data-delete-slot="${slot}" ${storage.read(slot).state ? "" : "disabled"}>刪除</button><button data-undelete-slot="${slot}" ${storage.deleted(slot).state && !storage.read(slot).state ? "" : "disabled"}>復原刪除</button><button data-backup-slot="${slot}" ${storage.readBackup(slot).state ? "" : "disabled"}>載入上次覆寫前</button></div>`).join("")}</details>${storage.sessionCopies().length ? `<details class="save-maintenance"><summary>離開頁面前保留的其他分頁進度</summary>${storage.sessionCopies().map(item => `<button data-session-copy="${esc(item.key)}">${esc(item.state.playerName)} · 第 ${item.state.life.game.week} 週</button>`).join("")}</details>` : ""}`;
  }
  ...
  async function handle(button) {
    ...
    if (d.confirmDelete) {
      api.toast(
        await storage.remove(d.confirmDelete)
          ? "已刪除，可從管理介面復原"
          : "無法刪除",
      );
      api.saves();
    ...
    if (d.undeleteSlot) {
      api.toast(
        await storage.restoreDeleted(d.undeleteSlot)
          ? "已復原存檔"
          : "無法復原或該位置已有存檔",
      );
      api.saves();
    ...
    if (d.sessionCopy) {
      const item = storage.readSessionCopy(d.sessionCopy);
    ...
    if (d.storage === "legacy") {
      ...
      api.show(
        "legacy-transfer",
        `${api.heading("WELCOME BACK", "接續舊版旅程", "只讀取這個瀏覽器的舊版存檔，不會覆寫它們。其他裝置可匯入先前備份的 JSON。")}${items.length ? items.map(({ key, item }) => `<button data-legacy-slot="${key}">${esc(item.state.playerName)} · 第 ${item.state.life.game.week} 週 · ${key === "star-game-save" ? "自動" : `手動 ${key.at(-1)}`}</button>`).join("") : "<p>這個網址來源沒有可搬移的舊版存檔，請使用匯入檔案。</p>"}<button data-ui="saves">返回</button>`,
      );
    }
    ...
    if (d.storage === "retire-confirm") {
      if (
        ...
        if (!await storage.backup(api.state())) {
        ...
        if (await api.checkpoint()) api.ending();
      }
    }
    return true;
  }
  ...
  return { extras, handle: async (button) => legacy(button) || await handle(button), file };
}
