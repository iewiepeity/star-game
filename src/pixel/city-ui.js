...
export function createCityUI(api) {
  ...
  function show(preselect = null) {
    ...
    api.show(
      "travel",
      `${api.heading("STARWISH CITY", "星望市", "點地圖上的建築選擇目的地。走訪免費，不消耗天數。")}<div class="map-tools"><label><span class="sr-only">尋找地點</span><input id="map-search" type="search" placeholder="找地點、課程或公司…" autocomplete="off"></label><button data-map-zoom="out" aria-label="縮小地圖">−</button><button data-map-zoom="in" aria-label="放大地圖">＋</button><button data-map-home>全市</button></div><div class="city-map-viewport" tabindex="0" aria-label="星望市地圖，可捲動"><div class="city-map-canvas" style="--map-zoom:${zoom}"><img class="city-map-art" src="assets/pixel/city/map.webp" alt="星望市：影視、音樂、傳媒、文化、生活與海灣街區"><nav class="city-map-landmarks" aria-label="城市地點">${CITY_PLACES.map((p) => `<button class="map-landmark ${publicRoom(s().sceneId) === p.id ? "current" : ""}" style="--x:${p.x}%;--y:${p.y}%" data-map-place="${p.id}" aria-label="${p.name}" aria-pressed="false"><span>${p.short}</span>${publicRoom(s().sceneId) === p.id ? '<i aria-hidden="true">▼</i>' : ""}</button>`).join("")}</nav><div class="map-watermark" aria-hidden="true">星望市<br><small>每一個轉角，都有新的日常</small></div></div></div><div id="map-detail" class="map-detail" aria-live="polite"><div><small>我的城市足跡</small><h3>${CITY_PLACES.filter((p) => s().visited.includes(p.id)).length} / 27</h3><p>移到建築上看看，或點一下選擇。手機可以放大、滑動地圖。</p></div><button data-ui="close">回到${ROOMS[s().sceneId].name}</button></div>`,
    );
