...
export function bindJobs() {
  ...
  document.querySelectorAll("[data-sequel-schedule]").forEach((button) => button.onclick = () => { const result = scheduleSequelSession(button.dataset.sequelSchedule); state.notice = result.message; if (result.ok) openApp(state,"planner"); render(); });
  document.querySelectorAll("[data-job-action]").forEach((button) => button.onclick = () => {
    ...
    if (action === "schedule-audition" || action === "retry") { const day = button.dataset.jobDay == null ? null : Number(button.dataset.jobDay), result = scheduleJobAudition(id, day); state.notice = result.message; if (result.ok) openApp(state,"planner"); }
    if (action === "sign") signJob(id);
    if (action === "schedule") { const day = button.dataset.jobDay == null ? null : Number(button.dataset.jobDay), result = scheduleJobSession(id, day); state.notice = result.message; if (result.ok) openApp(state,"planner"); }
