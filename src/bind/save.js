...
import{saveManualSlot,loadManualSlot,backupCurrent,loadBackup,exportSave,parseImportedSave,restoreRecentManualSlot}from"../core/persistence.js";
...
export function bindSave(){
 const showConfirm=(type,slot,trigger)=>{rememberDialogTrigger(trigger);state.confirmDialog={type,slot};renderUi()};
 const save=slot=>{const ok=saveManualSlot(slot,state,`手動存檔 ${slot}`);state.saveNotice=ok?`已儲存至槽位 ${slot}。`:"存檔寫入失敗，原本內容沒有被刪除。";renderUi()};
 document.querySelectorAll("[data-save-slot]").forEach(button=>button.onclick=()=>{const slot=Number(button.dataset.saveSlot);if(button.dataset.hasSave==="true"){showConfirm("overwrite",slot,button);return}save(slot)});
 ...
 document.querySelectorAll("[data-delete-slot]").forEach(button=>button.onclick=()=>showConfirm("delete-save",Number(button.dataset.deleteSlot),button));
