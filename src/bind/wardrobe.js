...
export function bindWardrobe(){
 document.querySelectorAll("[data-wardrobe-avatar]").forEach(x=>x.onclick=async()=>{const id=x.dataset.wardrobeAvatar;if(!AVATARS[id])return;await preloadImage(portraitAsset(id,state.outfitId));state.avatarId=id;state.wardrobeNotice="已切換人物立繪";render()});
 document.querySelectorAll("[data-outfit]").forEach(x=>x.onclick=async()=>{const id=x.dataset.outfit,outfit=OUTFITS[id];if(!outfit)return;if(!state.ownedOutfits.includes(id)){if(state.money<outfit.price){state.wardrobeNotice=`金額不足，購買「${outfit.name}」需要 $${outfit.price.toLocaleString("zh-TW")}`;render();return}state.money-=outfit.price;state.ownedOutfits.push(id);state.wardrobeNotice=`購買成功，已穿上「${outfit.name}」`}else state.wardrobeNotice=`已穿上「${outfit.name}」`;await preloadImage(portraitAsset(state.avatarId,id));state.outfitId=id;render()})
}
