...
export function bindMap(){
 document.querySelectorAll("[data-map-location]").forEach(x=>x.onclick=()=>{if(state.agencyInterview&&state.agencyInterview.dayIndex===state.selectedDay)cancelAgencyInterview();state.mapLocation=x.dataset.mapLocation;const day=state.selectedDay;state.schedule[day]="free";state.notice=`${DAYS[day]}將前往「${MAP_LOCATIONS[state.mapLocation].name}」`;state.selectedDay=(day+1)%7;state.appOpen="planner";render()});
}
