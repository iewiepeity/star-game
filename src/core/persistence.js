...
// SAVE_VERSION 是目前寫出的存檔格式版本號；SUPPORTED_SAVE_VERSIONS 則列出仍可由 hydrateState() 遷移的舊版。
// 升版時不要直接丟棄玩家進度：先讓讀檔接受舊版，再在 core/state.js 補齊欄位與轉換資料。
...
function readSlot(key){
 try{
  ...
  if(!parsed||!SUPPORTED_SAVE_VERSIONS.has(parsed.v)||!parsed.state)return null;
  return parsed;
 }catch(e){return null}
}
