...
export function parseImportedSave(text){const parsed=migrateEnvelope(JSON.parse(text));if(parsed?.game!=="star-game"||!parsed.state)throw new Error("不是可支援的《星途未定》存檔");return parsed.state}
