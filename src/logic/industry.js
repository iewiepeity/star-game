...
export function jobSource(job){const company=Object.values(INDUSTRY_COMPANIES).find(c=>c.categories.includes(job.category));if(!company)return{type:"public",company:null};const knownIndustryNpc=Object.values(state.relationships||{}).some(r=>(r.closeness||0)>=20||(r.trust||0)>=10);if((state.completedWorks?.length||0)>=8&&state.fame>=80)return{type:"direct",company};if(state.currentAgencyId)return{type:"agency",company};if(knownIndustryNpc&&job.stars>=3)return{type:"network",company};return{type:"public",company}}
...
export function canAccessJob(job){const source=jobSource(job),record=state.activeJobs[job.id];if(record)return{ok:true,source};if(source.type==="agency"||source.type==="network"||source.type==="direct")return{ok:true,source};return{ok:(state.visitedIndustryLocations||[]).includes(source.company?.locationId),source}}
