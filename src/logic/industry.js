...
export function canAccessJob(job){const source=jobSource(job),record=state.activeJobs[job.id];if(record)return{ok:true,source};if(["agency","network","direct"].includes(source.type))return{ok:true,source};return{ok:!!source.company&&hasRecentCastingIntel(source.company.locationId),source}}
