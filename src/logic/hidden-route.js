...
export const HIDDEN_ROUTE_CHAPTERS=Object.freeze([
 {id:"encounter",requires:()=>!known()&&state.week>=9&&state.familiarNpcs.length>=3,title:"錯過散場人潮之後",text:"電影院最後一批觀眾離開時，你和一名銀灰長髮的女子同時停下腳步。她看著你，像在確認一段只有夢裡才發生過的記憶。",beats:[{label:"散場廣播",text:"散場廣播催促觀眾離席，你們卻同時停在走道兩側。"},{label:"沒有來源的熟悉",text:"她準確說出你習慣先伸哪一隻手，說完後自己也怔住。"},{label:"重新自我介紹",text:"她叫沈霧棠。這一次，名字不是從任何舊記憶裡被撿回來的。"}],choices:[{id:"stay",label:"停下來問她的名字",outcome:"你們交換聯絡方式，誰也沒有把熟悉感當成承諾。",effect:npc(4,4,1)},{id:"leave",label:"先離開電影院",outcome:"她遞來名片，你們約定往後從正式的工作合作談起。",effect:npc(2,2)}]},
 ...
 scene("finale",68,"散場不再是唯一出口","新作品首映散場，觀眾陸續走出戲院。這一次，霧棠沒有站在門外催促；她和你一起留在走道，看著曾經象徵錯過的人潮慢慢離開。",[{label:"首映謝幕",text:"觀眾為那部沒有說明真假的作品鼓掌，沒有人需要知道它源自哪一輪。"},{label:"再次散場",text:"同樣的散場廣播響起，你們卻都沒有急著走出門。"},{label:"下一幕",text:"城市還亮著。你們決定用現在的時間，走去一個從未出現在舊分鏡裡的地方。"}],[{id:"together",label:"一起走向沒有畫過的下一幕",outcome:"熟悉感終於不再是債，而成為你們願意再次靠近的理由。",effects:[npc(8,10,8),{flag:"silver-route:present"}]},{id:"farewell",label:"好好道別，也保留這次相遇",outcome:"你們沒有成為戀人，卻完成了一段不需要以遺憾證明深刻的關係。",effects:[npc(5,9,0),{flag:"silver-route:farewell"}]}]),
]);
...
export function queueHiddenRoute(){
 ...
 const payload={...content,requires:{runMin:2},npcId:"silver_pc",choices,id:`silver-route-${chapter.id}`,kind:"人物事件",persistent:true,priority:110};
...
export function hiddenRoutePresentation(event) {
 if (!event || !["silver-route-encounter", "silver-route-finale"].includes(event.id)) return event;
 const chapter=HIDDEN_ROUTE_CHAPTERS.find(c=>`silver-route-${c.id}`===event.id);
 return {...event,npcId:"silver_pc",title:chapter.title,text:chapter.text,beats:chapter.beats,choices:event.choices?.map(choice=>{
