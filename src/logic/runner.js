...
import {
  ...
  markActivityDone,
  cancelActivity,
} from "./scheduled-activities.js";
import { resolvePersonalTask } from "./personal-tasks.js";
import { npcInteractionDecision } from "./npc-interaction-engine.js";
import {
  applyActivityLoad,
  healthPressure,
...
import { evaluateWeeklyTask } from "./weekly-task.js";
...
export function decisionFor(id) {
  if (id === "personal_task") {
    ...
    if (task?.kind === "npc_interact") return npcInteractionDecision(task);
    if (task?.kind === "manager_interact")
      return managerInteractionDecision(task);
  }
  if (id === "agency_interview" && state.agencyInterview) {
    const agency = AGENCIES[state.agencyInterview.agencyId];
    return {
      title: `${agency.name}面談`,
      text: `你打算怎麼呈現這次機會？目前${performanceLabel("audition")}。`,
      choices: [
        {
          id: "steady",
          label: "穩健回答",
          note: `${successRateLabel(successChanceWithCondition(deterministicInterviewScore(agency, "steady"), "audition"))}`,
        },
        {
          id: "bold",
          label: "展現企圖心",
          note: `${successRateLabel(successChanceWithCondition(deterministicInterviewScore(agency, "bold"), "audition"))}`,
        },
      ],
    };
  }
      ...
      return {
        ...
        choices: l.industry
          ? [
              {
                id: "browse_jobs",
                label: "進入 Casting Desk 查看徵選",
                note: "打開現場通告看板，符合資格可立即登記",
              },
              {
                id: "focus",
                label: "參觀公司與製作環境",
                note: "了解公司方向並獲得地點能力成長",
              },
            ]
          : [
              {
  ...
  if (id === "audition")
    return {
      title: "公開試鏡練習",
      text: `現場臨時要求即興；目前${performanceLabel("audition")}。`,
      choices: [
        { id: "safe", label: "照原本準備穩穩演", note: "成果穩定" },
        { id: "bold", label: "加入自己的設計", note: "風險較高" },
      ],
    };
  if (id === "street")
    return {
      title: "街頭的人比預期更多",
      text: `要怎麼抓住注意力？目前${performanceLabel("work")}。`,
      choices: [
        { id: "steady", label: "唱最熟悉的歌", note: "穩定演出" },
        { id: "interact", label: "主動邀請路人點歌", note: "考驗社交" },
      ],
    };
  return null;
}
export function applyGains(gains, multiplier = 1) {
  ...
  return (
    ...
    (state.focus === "growth" && out.length
      ? ` <em class="focus-result">✦ 策略「能力成長」已套用：每項額外＋1</em>`
      : "")
  );
}
function gainContractReadiness(id, success) {
  if (state.currentAgencyId || !["audition", "street"].includes(id)) return "";
  const gain = id === "audition" ? (success ? 3 : 1) : success ? 2 : 1;
  state.contract = Math.min(100, (state.contract || 0) + gain);
  return ` 簽約準備度＋${gain}。`;
}
...
export function advanceRunner() {
  clearRunnerTimer();
  if (state.screen !== "runner" || state.runnerPhase !== "result") return false;
...
function applyPersonalLoad(task) {
  if (!task) return;
...
function firstMeetingResult(encounters, progress) {
  const meetings = (encounters || []).filter(Boolean),
    first = meetings[0];
  if (!first) return null;
  const scenes = meetings
    .map(
      (meeting, index) =>
        `${index ? `<section class="additional-first-meeting"><h3>同一天認識的另一位合作對象・${meeting.npcName}</h3>` : ""}${meeting.text}${index ? "</section>" : ""}`,
    )
    .join("");
  return {
    title: first.title,
    text: `${scenes}<aside class="first-meeting-progress"><b>本日工作成果</b><span>${progress}</span></aside>`,
...
export function resolveDay(choice) {
  ...
  if (!workAccess(state, id).unlocked) {
    ...
    if (result.venue) title = `${result.venue.company.name}・Casting Desk`;
    if (result.encounter) {
      title = result.encounter.title;
      presentation = {
        ...presentation,
        npcName: result.encounter.npcName,
        portrait: result.encounter.portrait,
        accent: result.encounter.accent,
        requiresInteraction: true,
      };
    }
    ...
    title = result.ok
      ? result.title || task?.label || "個人安排完成"
      : "個人安排失敗";
    success = result.ok;
    text = result.text;
    presentation = {
    ...
    title =
      ...
      (result.completed
        ...
        : result.ok
          ...
          : "通告無法執行");
    success = result.ok;
    text = meeting?.text || progress;
    if (meeting)
      presentation = {
        npcName: meeting.npcName,
        portrait: meeting.portrait,
        accent: meeting.accent,
        requiresInteraction: meeting.requiresInteraction,
      };
    state.scheduledJobIds[day] = null;
  } else if (id === "agency_interview") {
    const active = state.agencyInterview,
      app = active && state.agencyApplications[active.agencyId],
      valid =
        active &&
        active.dayIndex === day &&
    ...
    if (!valid) {
      ...
      const agency = AGENCIES[active.agencyId],
      ...
      app.status = success ? "offer" : "rejected";
      if (success) {
        state.agencyOffer = { agencyId: agency.id, offeredWeek: state.week };
        title = "公司決定發出合約！";
      } else title = "這次沒有獲得錄取通知";
      text = `當日狀態修正後，機會評估為${successRateLabel(score)}。`;
    ...
    const power =
      ...
      base = Math.max(
        ...
        Math.min(
          ...
          32 +
            (power - 75) * 0.24 +
            (choice === "safe" || choice === "steady" ? 10 : -4),
        ),
      ),
      pct = Math.min(
        95,
  ...
  state.weekResults.push({
    day: DAYS[day],
    dayIndex: day,
    action: id === "personal_task" ? title || a.label : a.label,
    actionId: id,
    result: title + " " + text.replace(/<[^>]+>/g, ""),
    success,
  });
  if (applyHealthPressure()) return;
  state.runnerPhase = "result";
  state.runnerResult = { title, text, success, ...presentation };
  render();
  scheduleAutoAdvance();
}
export function hospitalize() {
  clearRunnerTimer();
  if (state.agencyInterview)
    cancelAgencyInterview("面談因住院取消，可以重新安排。");
  state.hospitalSkipWeeks = 0;
  state.forcedRestWeek = state.week + 1;
  state.flags ??= [];
  state.flags.push({
    week: state.week,
    label: "疲勞住院",
    note: "疲勞超過 100，本週剩餘行程取消，下一週強制休養。",
  });
  state.fatigue = Math.max(0, state.fatigue - 100);
  state.stamina = 100;
  state.health = Math.min(100, state.health + 30);
  for (let i = 0; i < 7; i++) {
    if (state.schedule[i] === "personal_task") cancelActivity(i);
    state.schedule[i] = "rest";
    state.freeLocations[i] = null;
    state.scheduledJobIds[i] = null;
    state.scheduledActivityIds[i] = null;
  }
  state.reward = {
    title: "身體拉起警報",
    text: "疲勞超過 100，本週剩餘行程全部取消；下一週必須完整休養，不能替換行程。",
  };
  finalizeWeekMemory({ hospitalized: true });
...
export function finishWeek() {
  ...
  if (state.forcedRestWeek === state.week) {
    state.reward = {
      title: "強制休養完成",
      text: "七天醫療休養已完成，身體狀態穩定下來；下週恢復正常排行程。",
    };
    const memory = finalizeWeekMemory({ hospitalized: true });
    state.history.push({
      week: state.week,
      results: structuredClone(state.weekResults),
      reward: 0,
      hospitalized: true,
      fullRecovery: true,
  ...
  state.history.push({
    ...
    results: structuredClone(state.weekResults),
    reward: task.money,
    weeklyTask: structuredClone(task),
    memory: structuredClone(memory),
  });
  state.screen = "summary";
  state.runnerDay = 0;
