...
import { applyFailureCallbackMechanics } from "./failure-depth.js";
...
function surfacePlayableDepth(p){const visible=[];
 if(p.chainStart)visible.push({id:p.chainStart.id,kind:"生活事件",priority:50,maxDelayWeeks:3,title:"片場的一件小事",text:p.chainStart.start,choices:(p.chainStart.choices||[]).map(c=>({id:c.id,label:c.label,outcome:"這個選擇現在看起來很小，但它不會在今天結束。",effect:{flag:`depth-chain:${p.chainStart.id}:${c.id}`}}))});
