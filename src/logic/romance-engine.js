...
function relationshipMessage(id,kind,text){if(!text)return;state.npcMessages??=[];state.npcMessages.push({id:`romance-status:${id}:${kind}:${state.week}:${state.relationships[id]?.romanceHistory?.length||0}`,week:state.week,npcId:id,title:`${NPCS[id].name}・${{public:"一起對外說明",underground:"保留私人空間",broken:"關係在這裡停下"}[kind]||"關係近況"}`,text,read:false,source:"romance"})}
const RUPTURE_LINES=Object.freeze({jiqing:"我不能繼續替這些傷害圓場。我們的伴侶關係，到這裡先結束。",shenyao:"這次的問題，不是再談一次就能當作沒有。我無法繼續這段伴侶關係。",tangtang:"我不想再笑著說沒事。我們分開吧，這次我是認真說的。",guchengxi:"我沒辦法再用沒關係帶過。我們的伴侶關係，到這裡停下來。",linxiafan:"我的底線已經說清楚了。這段關係不能照原來的方式繼續，我決定分開。",lujingran:"我在意的事，一直沒有被好好面對。我們分開吧。這次不是沉默讓你猜。",xiayutong:"我不能一直用下次會更好把問題跳過去。這段伴侶關係，我想停下來。",sufei:"這不是誰輸誰贏。但我不想再這樣相處，我決定分開。",chengyian:"我不能替眼前的傷害找一個好看的解釋。我們的伴侶關係，先在這裡結束。"});
...
export function transitionRomance(id,next,source="關係事件"){
 ...
 const before=current;rel.romance=next;rel.romanceSinceWeek=state.week;if(next==="dating"){state.partnerId=id;rel.visibility="underground"}if(PARTNER_STAGES.has(next))state.partnerId=id;if(["broken","rejected","none"].includes(next)&&state.partnerId===id)state.partnerId=null;if(next==="married")rel.visibility="public";rel.romanceHistory.push({week:state.week,from:before,to:next,source});rel.events=[...(rel.events||[]),{week:state.week,romance:next,source}];state.flags.push({week:state.week,label:`感情狀態：${NPCS[id].name}・${romanceStageLabel(next)}`,note:source});if(next==="broken"){const rupture=rel.hostility>=45,text=rupture?`${NPCS[id].name}明確告訴你：「${RUPTURE_LINES[id]||"我無法繼續這段伴侶關係。"}」`:NPC_ROMANCE_STATUS[id]?.broken;relationshipMessage(id,"broken",text+(before==="married"?" 你們決定分開生活，後續安排仍需要另行談清楚。":""))}return{ok:true,from:before,to:next,rel};
}

export function setRomanceVisibility(id,visibility,source="共同決定"){const rel=ensureRomanceFields(id);if(!PARTNER_STAGES.has(rel.romance)||!["underground","public"].includes(visibility))return{ok:false,reason:"目前沒有可調整公開狀態的戀愛關係。"};if(rel.visibility===visibility)return{ok:false,reason:"目前已經是這個狀態。"};rel.visibility=visibility;rel.romanceHistory.push({week:state.week,visibility,source});if(visibility==="public"){state.rep.話題度=Math.min(1000,(state.rep.話題度||0)+25);state.fans+=Math.max(20,Math.round((state.fans||0)*.02));const flag=`romance:public:${id}`;if(!state.eventFlags.includes(flag))state.eventFlags.push(flag)}relationshipMessage(id,visibility,NPC_ROMANCE_STATUS[id]?.[visibility]?.replaceAll("交往",rel.romance==="married"?"結婚":"交往"));return{ok:true,visibility}}
