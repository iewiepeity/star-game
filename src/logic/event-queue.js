...
function eventPersists(event, source) {
  return event?.persistent || eventPriority(event, source) >= 80;
}

export function queueEvent(
  event,
  { source = "系統", dueWeek = state.week } = {},
) {
  if (!event || eventExists(event)) return false;
  const queuedWeek = state.week,
    expiresWeek = eventPersists(event, source)
      ? null
      : dueWeek + (event.maxDelayWeeks ?? 10);
  state.queuedEvents.push({
    ...
    queuedWeek,
    expiresWeek,
    priority: eventPriority(event, source),
  });
  return true;
}
...
export function enqueueVisibleEvent(event, source = "系統", meta = {}) {
  ...
  if (
    ...
    const queuedWeek = meta.queuedWeek ?? state.week,
      expiresWeek =
        meta.expiresWeek ??
        (eventPersists(event, source)
          ? null
          : queuedWeek + (event.maxDelayWeeks ?? 10));
    ...
    state.queuedEvents.push({
      ...
      queuedWeek,
      expiresWeek,
      priority: meta.priority ?? eventPriority(event, source),
    });
    return "deferred";
  }
  state.eventQueue.push({
    event,
    source,
    queuedWeek: meta.queuedWeek ?? state.week,
    priority: meta.priority ?? eventPriority(event, source),
  });
...
export function obsoleteMediaEvent(event) {
  const id =
    event?.mediaRomanceNpc ||
    event?.choices
      ?.flatMap((c) => c.effects || [])
      .find(
        (e) =>
          e.flag?.startsWith("rumor-response:") && e.flag.endsWith(":confirm"),
      )
      ?.flag?.split(":")[1];
  const npc =
    event?.mediaRomanceNpc ||
    (state.rumors || []).find((r) => r.id === id)?.npcId;
  return (
    !!npc &&
    (state.relationships?.[npc]?.visibility === "public" ||
      (state.rumors || []).some(
        (r) =>
          r.npcId === npc &&
          (r.response === "confirm" ||
            state.eventFlags?.includes(`rumor-response:${r.id}:confirm`)),
      ))
  );
}
