...
export function socialDrafts(){
 ...
 if(work)drafts.afterwork={label:"作品幕後",icon:"幕",text:`${titleTag(work.title)}完成後，我最想記住的不是結果，而是現場那次選擇：${work.storyLegacy||"把每個小決定做到誠實"}。`};
 if(creative)drafts.original={label:"原創進度",icon:"創",text:`${titleTag(creative.title)}正在走自己的路。${creative.status==="sold"?"這次選擇把企劃交給公司，也學會創作權是有重量的。":creative.status==="released"?`成品是 ${creative.finalGrade?.grade||"-"} 級，但數字之外還有我想留下的方向。`:"製作現場正在把紙上的想法變成真正的作品。"}`};
