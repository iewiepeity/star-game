...
import{WEEKLY_THREADS,SHORT_CHAINS,CREW_ROLES,CREW_SURNAMES,CREW_GIVEN,AUDITION_INTEL,LEGACY_TAGS,MEMORY_LABELS,FAME_LIFE_TIERS,SCANDAL_RESPONSES,ANNUAL_TENTPOLES,NG_PLUS_INTUITION}from"../data/playable-depth-content.js";
...
export function crewForWork(work){ensurePlayableDepthState();if(!work)return[];if(state.crewNetwork[work.id])return state.crewNetwork[work.id];const roles=work.category==="歌曲"?["音樂製作人","造型師"]:work.category==="綜藝"?["製作人","主持人"]:work.category==="廣告"?["導演","造型師"]:["導演","合作演員","攝影指導"];const crew=roles.map((role,i)=>{const key=`${work.id}-${role}-${i}`,name=pick(CREW_SURNAMES,key)+pick(CREW_GIVEN,key+"g");const prior=Object.values(state.crewNetwork).flat().find(p=>p.name===name);return{id:`crew-${hash(name+role)}`,role,name,trust:prior?.trust||0,metWeek:prior?.metWeek||state.week,works:[...(prior?.works||[]),work.id]}});state.crewNetwork[work.id]=crew;return crew}
...
export function tickPlayableDepth(){ensurePlayableDepthState();state.playTelemetry.weeks++;const thread=buildWeeklyThread(),chainStart=maybeStartShortChain(),chains=tickShortChains(),callbacks=tickFailureCallbacks(),manager=managerIntervention(),romance=romanceWorkPressure(),tentpole=annualTentpoleStatus(),fame=fameLifeTier(),intuition=ngPlusIntuition();syncWorkLegacies();syncRelationshipMemories();return{thread,chainStart,chains,callbacks,manager,romance,tentpole,fame,intuition}}
