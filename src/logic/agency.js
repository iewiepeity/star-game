import { agencyContactsUnlocked } from "./city-progression.js";
...
export function canApplyToAgency(agency){if(!agency || !agencyContactsUnlocked(state))return false;if(isAgencyContractActive())return false;const app=state.agencyApplications[agency.id];if(app&&!['rejected','expired','declined'].includes(app.status))return false;return checkAgencyEligibility(agency).met}
