...
const validateEffect=(effect,where)=>{if(!effect)return;const hiddenName=effect.hidden===true?effect.stat:(typeof effect.hidden==="string"?effect.hidden:null),repName=effect.rep===true?effect.stat:(typeof effect.rep==="string"?effect.rep:null);if(hiddenName)assert(hidden.has(hiddenName),`${where} 使用未知隱藏特質 ${hiddenName}`);else if(repName)assert(rep.has(repName),`${where} 使用未知評價 ${repName}`);else if(effect.stat)assert(ABILITIES.includes(effect.stat),`${where} 使用未知能力 ${effect.stat}`);if(effect.npc)assert(NPCS[effect.npc],`${where} 使用未知 NPC ${effect.npc}`)};
