...
const paths=["pixel.html","pixel.css","pixel-ui.css","pixel-apps.css","pixel-theme.css","pixel.webmanifest",...(await walk("src/")).filter(p=>p.endsWith(".js")),...(await walk("assets/")).filter(p=>/\.(webp|jpg|jpeg|png|svg|ogg|js|json|woff2|txt)$/i.test(p))];
const entries=await Promise.all(paths.map(async url=>({url:"./"+url,bytes:(await stat(new URL(url,root))).size})));
await writeFile(new URL("pixel-offline.json",root),JSON.stringify({version:"0.9.0",bytes:entries.reduce((s,e)=>s+e.bytes,0),entries},null,2)+"\n");
